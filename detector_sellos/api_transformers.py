...
app = FastAPI(title="Detector de Sellos API - Transformers")
...
def health():
    return {
        ...
        "message": "API funcionando con Transformers"
    }
    ...
    try:
        ...
        for svg in svgs:
            ...
            try:
                ...
                # Agregar fondo blanco si es necesario
                ...
                png_to_svg_name[os.path.basename(png_path)] = svg.filename
        ...
        for foto in fotos:
            ...
            try:
                ...
                for png_path in png_temp_paths:
                    try:
                        svg_image = Image.open(png_path).convert("RGB")
                        ...
                        nombre_svg = png_to_svg_name.get(os.path.basename(png_path), os.path.basename(png_path))
                        mejores_matches.append((nombre_svg, similarity))
                        
                    except Exception as e:
                        print(f"Error procesando {png_path}: {e}")
                        continue
        ...
        # Limpiar archivos temporales
        for path in svg_temp_paths + png_temp_paths:
            if os.path.exists(path):
                os.unlink(path)

        return {
            "success": True,
            "message": "Análisis completado con Transformers",
            "results": resultados
        }
