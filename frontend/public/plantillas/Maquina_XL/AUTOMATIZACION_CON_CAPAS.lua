...
-- VERSIÓN PARA MÁQUINA XL - Columna única con optimización de material
...
-- CONFIGURACIÓN ESPECÍFICA PARA MÁQUINA XL
local XL_COLUMN_X = 31.5  -- Posición X de la columna única
local XL_COLUMN_WIDTH = 63 -- Ancho máximo de la columna en mm
local XL_START_Y = -1  -- Posición Y inicial
local XL_SEPARATION = 4    -- Separación entre vectores en mm
...
function ProcessSingleObject(obj)
    ...
    -- OPTIMIZACIÓN PARA MÁQUINA XL: Rotar para maximizar el uso del ancho de columna
    ...
    -- Determinar si necesita rotación para optimizar material
    local needRotation = false
    if groupHeight > groupWidth and groupHeight <= XL_COLUMN_WIDTH then
        -- El lado más largo (altura) cabe en el ancho de columna, rotar
        needRotation = true
    elseif groupWidth > groupHeight and groupWidth <= XL_COLUMN_WIDTH then
        -- El lado más largo (ancho) ya está en la dirección correcta
        needRotation = false
    elseif groupHeight <= XL_COLUMN_WIDTH and groupWidth <= XL_COLUMN_WIDTH then
        -- Ambos lados caben, usar el más largo en el ancho
        needRotation = (groupHeight > groupWidth)
    else
        -- Ningún lado cabe, usar el más pequeño en el ancho
        needRotation = (groupHeight < groupWidth)
    end
    ...
    if needRotation then
        ...
        -- Recalcular dimensiones después de rotación
        groupWidth = bbox.MaxX - bbox.MinX
        groupHeight = bbox.MaxY - bbox.MinY
    end
    
    -- Verificar que el ancho no exceda el límite de la columna
    if groupWidth > XL_COLUMN_WIDTH then
        DisplayMessage("⚠ Advertencia: El vector es más ancho que la columna (" .. groupWidth .. "mm > " .. XL_COLUMN_WIDTH .. "mm)")
        DisplayMessage("→ Se posicionará de todas formas, pero puede causar problemas")
    end
    
    -- Determinar posición Y evitando superposiciones (secuencial en columna única)
    ...
    if corteLayer then
        ...
        while pos2 do
            ...
            if obj then
                ...
                if objBBox then
                    -- En columna única, solo verificar posición Y
    ...
    -- Posicionar en la columna única (centrado en X = 31.5)
    local groupCenterX = (bbox.MinX + bbox.MaxX) / 2
    ...
    if updated_bbox then
        -- Crear elementos adicionales específicos para XL
...
function CreateAdditionalElementsXL(bbox)
    -- Validar parámetros
    if not bbox then
        DisplayMessage("Error: CreateAdditionalElementsXL - parámetros inválidos")
        return
    end
    ...
    if not bbox.MinX or not bbox.MaxX or not bbox.MinY or not bbox.MaxY then
        DisplayMessage("Error: CreateAdditionalElementsXL - bbox inválido")
        return
    end
    
    -- Línea horizontal para XL (ancho completo de la columna)
    ...
    if not lineX1 or not lineX2 or not lineY or 
       lineX1 ~= lineX1 or lineX2 ~= lineX2 or lineY ~= lineY then -- NaN check
        DisplayMessage("Error: CreateAdditionalElementsXL - valores de línea inválidos")
        return
    end
    ...
    if not circleCenterX or not circleCenterY or 
       circleCenterX ~= circleCenterX or circleCenterY ~= circleCenterY then -- NaN check
        DisplayMessage("Error: CreateAdditionalElementsXL - valores de círculo inválidos")
        return
    end
...
function ProcessStep3RectangleOperations()
    ...
    -- DEFINICIÓN DE COLUMNA ÚNICA PARA MÁQUINA XL
    local column = {
        nom = XL_COLUMN_WIDTH,  -- Ancho nominal de la columna
        xpos = XL_COLUMN_X      -- Posición X de la columna
    }

    local rectangles_created = 0

    -- Buscar vectores en la columna única
    local minY = nil
    ...
    while pos do
        ...
        if obj then
            ...
            if bb then
                ...
                if math.abs(cx - column.xpos) < tol then
    ...
    if minY then
        ...
        -- Crear Rectángulo 1: ancho = (nominal + 6) mm, centrado en column.xpos
        local width1 = column.nom + 6
        local halfWidth1 = width1 / 2
        local leftX1 = column.xpos - halfWidth1
        local rightX1 = column.xpos + halfWidth1
        ...
        -- Crear Rectángulo 2: ancho = (nominal + 14) mm, centrado en column.xpos
        local width2 = column.nom + 14
        local halfWidth2 = width2 / 2
        local leftX2 = column.xpos - halfWidth2
        local rightX2 = column.xpos + halfWidth2
            ...
            if ProcessStep3RectangleOperations() then
                ...
                if UngroupAllVectors() then
                    ...
                    if ProcessStep4RecalculateToolpaths() then
                        ...
                        DisplayMessage("🏭 MÁQUINA XL - Columna única optimizada")
                        DisplayMessage("📏 Columna: X=" .. XL_COLUMN_X .. "mm, Ancho=" .. XL_COLUMN_WIDTH .. "mm")
                        return true
                    else
                        DisplayMessage("❌ Error recalculando trayectorias")
