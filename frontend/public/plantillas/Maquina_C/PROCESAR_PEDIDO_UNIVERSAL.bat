@echo off
title Procesando Pedido - Aspire Automatico (Maquina C)
color 0A
echo ==========================================
...
REM Buscar el archivo .lua para máquina C
set "LUA_SCRIPT="
for %%f in ("%CURRENT_DIR%AUTOMATIZACION_CON_CAPAS.lua") do set "LUA_SCRIPT=%%f"

if "%LUA_SCRIPT%"=="" (
    echo ERROR: No se encontro el script para maquina C
...
set "TEMP_LUA=%TEMP%\AUTOMATIZACION_C_TEMP.lua"
...
echo Ejecutando automatizacion para MAQUINA C...
...
echo.
echo ==========================================
echo    INFORMACION DE LA MAQUINA C
echo ==========================================
echo.
echo Configuracion estandar para Maquina C:
echo - Coordenadas originales del script
echo - Sistema de columnas estandar
echo - Posicionamiento optimizado para maquina C
echo.
